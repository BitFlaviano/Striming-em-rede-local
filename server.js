  ...
  for (const name of Object.keys(interfaces)) {
    for (const iface of interfaces[name]) {
      if (iface.family === 'IPv4' && !iface.internal) {
        ...
        candidates.push({ ip: iface.address, prefix, name, priority: prefix === gatewayPrefix ? 0 : 1 });
      }
    }
  }

  // Sort: matching gateway subnet first, then by name priority (Ethernet/Wi-Fi first)
  candidates.sort((a, b) => {
