(function() {
  ...
  function cacheDom() {
    ...
    DOM.btnRewind = document.getElementById('btn-rewind');
    DOM.btnForward = document.getElementById('btn-forward');
  ...
  function setupVideoControls() {
    ...
    v.addEventListener('timeupdate', () => {
      ...
      if (!fullscreenTimer && v.currentTime >= 20 && !document.fullscreenElement && !v.paused) {
    ...
    v.addEventListener('play', () => {
      DOM.btnPlayPause.textContent = '⏸';
      isPlaying = true;
      if (v.currentTime >= 20 && !document.fullscreenElement && fullscreenTimer) {
  ...
  async function init() {
    ...
    DOM.btnRewind.addEventListener('click', () => seekRelative(-10));
    DOM.btnForward.addEventListener('click', () => seekRelative(10));
    ...
    try {
      ...
      if (info.addresses.length > 0) {
        const ip = info.addresses[0].address;
        DOM.serverInfo.textContent = `${info.hostname} (${ip})`;
        DOM.serverIpDisplay = document.getElementById('server-ip-display');
        DOM.serverBadge = document.getElementById('server-badge');
        DOM.btnCopyIp = document.getElementById('btn-copy-ip');
        if (DOM.serverIpDisplay) {
          DOM.serverIpDisplay.textContent = ip;
          DOM.serverBadge.classList.remove('hidden');
          DOM.btnCopyIp.classList.remove('hidden');
          DOM.btnCopyIp.addEventListener('click', () => {
            navigator.clipboard.writeText(ip);
            showError('IP copiado: ' + ip, 1500);
          });
        }
      }
    } catch (_) {
